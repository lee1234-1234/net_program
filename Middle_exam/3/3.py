        ...
        if len(tokens) >= 2:
            ...
            if filename.startswith("exam"):
                try:
                    ...
                    resp_body = f"<html><head><title>Hello</title></head><body>Hello, {query}</body></html>"
                    resp = resp_line + resp_headers + resp_body
                    c.send(resp.encode())
                except Exception as e:
                    print("Error processing 'exam':", e)
                    resp_line = "HTTP/1.1 500 Internal Server Error\r\n"
                    resp_headers = "\r\n"
                    resp_body = "<html><head><title>Error</title></head><body>Internal Server Error</body></html>"
                    resp = resp_line + resp_headers + resp_body
                    c.send(resp.encode())
            else:
                try:
